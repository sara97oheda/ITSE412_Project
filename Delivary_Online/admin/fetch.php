...
$columns = array('username', 'passWord','Email','typeWork');
...
if(isset($_POST["search"]["value"]))
{
 ...
 OR typeWork LIKE "%'.$_POST["search"]["value"].'%" 
 ';
}
...
while($row = mysqli_fetch_array($result))
{
    ...
    $sub_array[] = '<div contenteditable class="update" data-id="'.$row["id"].'" data-column="typeWork">' . $row["typeWork"] . '</div>';
    $sub_array[] = '<button type="button" name="delete" class="btn btn-danger btn-xs delete" id="'.$row["id"].'">حذف</button>';
    $data[] = $sub_array;
}
